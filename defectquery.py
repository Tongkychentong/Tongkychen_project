#--*-- coding:utf-8 --*--
# 背景:现缺陷的量级及打点日期进行统计，明确缺陷状态进行后续的下缺陷方案制定
# 输入：多级目录的sq3文件，输出：excel的统计结果。
# 方案：
# 通过提取缺陷图层，分类型分issueid维度筛出日期比较靠前的量级。量级包含日期，处。
# 1.遍历多级目录，查找.sq3文件，判断defectinfo图层和patchinfo图层是否存在记录，如果存在记录通过所在的文件名称去B目录找到相同名称的文件。
